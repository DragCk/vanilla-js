...
export class Car{
    ...
    #assessDamage(borders, traffic){
        for(let i=0 ; i < borders.length ; i++){
            if(Operations.polysIntersect(this.polygon, borders[i])) return true
        }
        for(let i=0 ; i < traffic.length ; i++){
            if(Operations.polysIntersect(this.polygon, traffic[i].polygon)) return true
        }
        return false
    }
    ...
    #move(){
        if(this.controls.foward) this.speed += this.acceleration
